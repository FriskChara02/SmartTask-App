        ...
        NavigationStack {
                    ...
                    MessageInputView(
                        ...
                        sendAction: {
                            guard let userId = authVM.currentUser?.id else {
                                chatVM.alertMessage = "Vui lòng đăng nhập để gửi tin nhắn .ᐟ"
                                chatVM.showAlert = true
                                return
                            }
                            if !messageText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                                chatVM.sendSmartTaskMessage(userId: userId, content: messageText)
                                messageText = ""
                            }
                        }
                    )
            ...
            .onAppear {
                if let userId = authVM.currentUser?.id {
                    chatVM.fetchSmartTaskMessages(userId: userId)
                } else {
                    chatVM.alertMessage = "Vui lòng đăng nhập để sử dụng SmartTaskChat .ᐟ"
                    chatVM.showAlert = true
                }
            }
...
#Preview {
    ...
    let chatVM = ChattingViewModel(authVM: authVM)
    ...
    chatVM.smartTaskMessages = [
        ChatMessage(
            ...
            name: "Test User",
            avatarURL: nil,
            content: "SmartTask là gì?" as String?,
        ...
        ChatMessage(
            ...
            name: "SmartTask",
            avatarURL: nil,
            content: "SmartTask là một ứng dụng quản lý công việc và thời gian biểu thông minh..." as String?,
